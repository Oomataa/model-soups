...
from milkshake.datamodules.dataset import Subset, BootstrapSampler
...
class DataModule(VisionDataModule):
    ...
    def __init__(self, args, dataset_class, num_classes, num_groups):
        ...
        self.bootstrap = getattr(args, "bootstrap", False)
        self.bootstrap_frac = getattr(args, "bootstrap_frac", 0.8)
        self.bootstrap_n_sets = getattr(args, "bootstrap_n_sets", 5)
        self.bootstrap_with_replacement = getattr(args, "bootstrap_with_replacement", False)
    ...
    def _split_dataset(self, dataset, val=False):
        ...
        if self.bootstrap:
            ...
            dataset_train = BootstrapSampler(
            dataset_train, 
            frac= self.args.bootstrap_frac, 
            n_sets= self.args.bootstrap_n_sets, 
            with_replacement= self.args.bootstrap_with_replacement, 
            seed= self.args.seed
        
        )
            print("The code is using bootstrap")
